  ...
  container.innerHTML = vaultData.map(v => `
    <div class="job" style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap" data-id="${v.id}">
      ...
      <div style="display:flex;gap:8px">
        <a class="btn small" href="/api/vault/${v.id}/download" download>\uD83D\uDCE5 Download</a>
...
function aktualisiereVaultSelects() {
  const opts = '<option value="">\u2014 kein CV verkn\u00fcpft \u2014</option>' +
    vaultData.map(v => `<option value="${v.id}">${esc(v.name)}</option>`).join('');

  // Alle inline .vault-select Felder (Bearbeiten-Zeile)
  ...
  // Festes Modal-Select
  const mSel = document.getElementById('mVaultSelect');
  if (mSel) {
...
window._vaultUpload = async function() {
  ...
  if (!file) return window.toast && window.toast('Bitte eine Datei w\u00e4hlen', 'warn');
  const fd = new FormData();
  fd.append('datei', file);
  fd.append('name', name || file.name);
  if (notiz) fd.append('notiz', notiz);
  const r = await fetch('/api/vault', { method:'POST', body: fd });
  if (!r.ok) { const e = await r.json(); return window.toast && window.toast('Fehler: '+(e.error||r.status), 'error'); }
