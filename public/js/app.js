...
async function saveVorlage(){await api('/api/vorlagen','POST',{name:$('vorlageName').value,einleitung:$('vorlageEinleitung').value,schluss:$('vorlageSchluss').value});loadVorlagen();log('Vorlage gespeichert.');}
...
async function loadKiModels() {
  try {
    ...
    sel.innerHTML = (data.models || ['mistral']).map(m =>
      `<option value="${m}" ${m === data.active ? 'selected' : ''}>${m}</option>`
    ).join('');
  } catch(e) { console.warn('Ollama nicht erreichbar:', e.message); }
}
...
async function kiAnschreibenErzeugen(jobIndex) {
  ...
  try {
    const res = await fetch('/api/ki/anschreiben', {
      method: 'POST',
...
async function init(){
  ...
  loadKiModels();
  loadQuellenStatus();
  setInterval(loadBewerbungen, 5*60*1000);
  log('JobRadar bereit.');
}
